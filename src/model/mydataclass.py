...
# 单个 motif 拼接任务的数据（一个图 + 位置 label 等）
@dataclass
class train_data:
    graph: Data  # 分子图
    query_atom: int # 当前要连接的原子编号
    cyclize_cand: List[int]  # 环化候选连接点
    label: Tuple[int, int]  # label = (motif_id, attach_point_id)

# 单个分子对应的数据（图、性质、起始点标签等）
@dataclass
class mol_train_data:
    mol_graph: Data  # 整体分子图结构
    props: Tensor  # 分子性质标签
    start_label: int  # 起始 motif 的分类标签
    train_data_list: List[train_data]  # 每一步拼接的数据
    motif_list: List[int]  # 所用的 motif 编号序列

# batch 化后的训练数据结构（用于模型 forward）
@dataclass
