...
class MolGraph(object):
    ...
    # 应用 motif BPE 操作合并图节点，生成 merging_graph
        mol = self.mol
    ...
    # 根据 merging_graph 标记连接点、添加 dummy atom 等
        mol_graph = self.mol_graph.copy()
    ...
    def relabel(self):
    # 对 merging_graph 中每个 motif 计算其连接点顺序、标号等"""
    ...
    def get_data(self) -> mol_train_data:
        # 构造训练用数据结构 mol_train_data，包括拼接步骤轨迹等"""
        self.relabel()
    ...
    # 将所有 vocab motif 转换为图，并 batch 成批量数据结构"""
        vocab_data = []
    ...
    def motif_to_graph(smiles: str, motif_list: Optional[List[str]] = None) -> Tuple[nx.Graph, List[int], List[int]]:
        # 将 motif smiles 转换为 graph + dummy + ranks"""
