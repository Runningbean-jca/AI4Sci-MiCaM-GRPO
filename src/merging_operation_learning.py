...
# 通过遍历训练集中的分子结构图，学习出一系列高频 motif 合并操作（merging operations），用于构建 motif vocabulary。这些操作被记录在 merging_operation.txt 文件中，用于在后续 MolGraph 中进行 motif 分割。
@dataclass
class MolGraph:
    idx: int
...
def merging_operation_learning(
    ...
    cooccur = defaultdict(lambda: defaultdict(int))  # <--- 新增：用于统计 motif 共现
    ...
    for i in range(num_iters):
        ...
        # 在 motif 被应用后，统计 motif 与邻居 motif 的共现连接
        for mol in mols:
            for node in mol.merging_graph.nodes:
                neighbors = list(mol.merging_graph.neighbors(node))
                for n in neighbors:
                    atoms1 = mol.merging_graph.nodes[node]["atom_indices"]
                    atoms2 = mol.merging_graph.nodes[n]["atom_indices"]
                    smi1 = fragment2smiles(mol, atoms1)
                    smi2 = fragment2smiles(mol, atoms2)
                    a, b = sorted([smi1, smi2])
                    if a != b:
                        cooccur[a][b] += 1
    ...
    # 在最终返回前将 cooccur 写入 JSON 文件
    with open(os.path.join(dir, "motif_cooccur.json"), "w") as f:
    ...
    print(f"Motif co-occurrence stats written to motif_cooccur.json.\n")
